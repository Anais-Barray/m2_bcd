...
void readAAMasses(string fName, vector<double>& aaMasses, double& water, double& proton)
{
...
void printDigestProduct(const string& protein, vector<PeptidePosition>& pept)
{
  for (int i = 0; i < pept.size(); i++)
    cout << protein.substr(pept[i].first, pept[i].length) << "\t" << pept[i].nmc << "\t" << getPeptideMass(protein, pept[i]) << endl;
...
int main(int argc, char* argv[])
{
  ...
  {
    if ((strcmp(argv[i], "-protein") == 0) && (i < argc-1))
      protein = string(argv[++i]);
  ...
  printDigestProduct(protein, pept);
